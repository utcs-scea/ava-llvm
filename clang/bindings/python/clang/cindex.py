...
class TranslationUnitSaveError(Exception):
    ...
    def __init__(self, enumeration, message):
        ...
        if enumeration < 1 or enumeration > 3:
            raise RuntimeError("Encountered undefined TranslationUnit save error "
                               "constant: %d. Please file a bug to have this "
                               "value supported." % enumeration)

        self.save_error = enumeration
        super(TranslationUnitSaveError, self).__init__(self, 'Error %d: %s' % (enumeration, message))
...
class Cursor(Structure):
    ...
    def pretty_printed(self, policy=None):
        """Pretty print declarations."""
        return conf.lib.clang_getCursorPrettyPrinted(self, policy)
    ...
    @property
    def printing_policy(self):
        """Retrieve the default policy for the cursor."""
        return conf.lib.clang_getCursorPrintingPolicy(self)
    ...
    @property
    def referenced_name_range(self):
        return conf.lib.clang_getCursorReferenceNameRange(self, 0x4, 0)
...
class Type(Structure):
    ...
    def element_type(self):
        ...
        if result.kind == TypeKind.INVALID:
            raise ValueError('Element type not available on this type: ' + self.kind.name)
    ...
    def element_count(self):
        ...
        if result < 0:
            raise ValueError('Type does not have elements: ' + self.kind.name)
...
class ClangObject(object):
    ...
    def __init__(self, obj):
        assert isinstance(obj, c_object_p) and obj, obj
...
def _PrintingPolicy_make_property(property_id, type = bool, doc = None):
    def getter(self):
        res = conf.lib.clang_PrintingPolicy_getProperty(self, property_id)
        if type:
            return type(res)
        return res
    def setter(self, value):
        return conf.lib.clang_PrintingPolicy_setProperty(self, property_id, int(value))
    return property(getter, setter, doc = doc)


class PrintingPolicy(ClangObject):
    """
    The PrintingPolicy class represents a policy that controls pretty printing
    for Curser.pretty_printed(policy).
    """

    indentation = _PrintingPolicy_make_property(0, int, doc="The number of spaces to use to indent each line.")
    suppress_specifiers = _PrintingPolicy_make_property(1, doc="Whether we should suppress printing of the actual specifiers for the given type or declaration.")
    suppress_tag_keyword = _PrintingPolicy_make_property(2, doc="Whether type printing should skip printing the tag keyword.")
    include_tag_definition = _PrintingPolicy_make_property(3, doc="When true, include the body of a tag definition.")
    suppress_scope = _PrintingPolicy_make_property(4, doc="Suppresses printing of scope specifiers.")
    suppress_unwritten_scope = _PrintingPolicy_make_property(5, doc="Suppress printing parts of scope specifiers that don't need to be written, e.g., for inline or anonymous namespaces.")
    suppress_initializers = _PrintingPolicy_make_property(6, doc="Suppress printing of variable initializers.")
    constant_array_size_as_written = _PrintingPolicy_make_property(7, doc="Whether we should print the sizes of constant array expressions as written in the sources.")
    ananymous_tag_locations = _PrintingPolicy_make_property(8, doc="""When printing an anonymous tag name, also print the location of that entity (e.g., doc="enum <anonymous at t.h:10:5>"). Otherwise, just prints "(anonymous)" for the name.""")
    suppress_strong_lifetime = _PrintingPolicy_make_property(9, doc="When true, suppress printing of the __strong lifetime qualifier in ARC.")
    suppress_lifetime_qualifiers = _PrintingPolicy_make_property(10, doc="When true, suppress printing of lifetime qualifier in ARC.")
    suppress_template_args_in_CXX_constructors = _PrintingPolicy_make_property(11, doc="When true, suppresses printing template arguments in names of C++ constructors.")
    use_bool = _PrintingPolicy_make_property(12, doc="Whether we can use 'bool' rather than '_Bool' (even if the language doesn't actually have 'bool', because, e.g., it is defined as a macro).")
    restrict = _PrintingPolicy_make_property(13, doc="Whether we can use 'restrict' rather than '__restrict'.")
    alignof = _PrintingPolicy_make_property(14, doc="Whether we can use 'alignof' rather than '__alignof'.")
    underscore_alignof = _PrintingPolicy_make_property(15, doc="Whether we can use '_Alignof' rather than '__alignof'.")
    use_void_for_zero_params = _PrintingPolicy_make_property(16, doc="Whether we should use '(void)' rather than '()' for a function prototype with zero parameters.")
    terse_output = _PrintingPolicy_make_property(17, doc="Provide a 'terse' output.")
    polish_for_declaration = _PrintingPolicy_make_property(18, doc="When true, do certain refinement needed for producing proper declaration tag; such as, do not print attributes attached to the declaration.")
    half = _PrintingPolicy_make_property(19, doc="When true, print the half-precision floating-point type as 'half' instead of '__fp16'")
    MS_wchar = _PrintingPolicy_make_property(20, doc="When true, print the built-in wchar_t type as __wchar_t. For use in Microsoft mode when wchar_t is not available.")
    include_newlines = _PrintingPolicy_make_property(21, doc="""When true, include newlines after statements like "break", etc.""")
    MSVC_formatting = _PrintingPolicy_make_property(22, doc="Use whitespace and punctuation like MSVC does. In particular, this prints anonymous namespaces as `anonymous namespace' and does not insert spaces after template arguments.")
    constants_as_written = _PrintingPolicy_make_property(23, doc="Whether we should print the constant expressions as written in the sources.")
    suppress_implicit_base = _PrintingPolicy_make_property(24, doc="When true, don't print the implicit 'self' or 'this' expressions.")
    fully_qualified_name = _PrintingPolicy_make_property(25, doc="When true, print the fully qualified name of function declarations. This is the opposite of SuppressScope and thus overrules it.")

    def __del__(self):
        conf.lib.clang_PrintingPolicy_dispose(self)

    def __repr__(self):
        return "<PrintingPolicy: %s>" % (self.obj)

    @staticmethod
    def from_result(res, fn, args):
        assert isinstance(res, c_object_p)
        return PrintingPolicy(res)
...
class TranslationUnit(ClangObject):
    ...
    def get_extent(self, filename, locations):
        ...
        if len(locations) < 2:
            raise ValueError('Must pass object with at least 2 elements')
...
class File(ClangObject):
    ...
    def __eq__(self, other):
        return self is other or self.name == other.name and self.time == other.time

    def __hash__(self):
        return hash(self.name) + hash(self.time)
...
class CompilationDatabaseError(Exception):
    ...
    def __init__(self, enumeration, message):
        ...
        if enumeration > 1:
            raise RuntimeError("Encountered undefined CompilationDatabase error "
                            "constant: %d. Please file a bug to have this "
                            "value supported." % enumeration)

        self.cdb_error = enumeration
        super(CompilationDatabaseError, self).__init__(self, 'Error %d: %s' % (enumeration, message))
...
#  [CXTUResourceUsage]),
  ...
  ("clang_getCursorPrintingPolicy",
   [Cursor],
   c_object_p,
   PrintingPolicy.from_result),

  ("clang_PrintingPolicy_dispose",
   [PrintingPolicy]),

  ("clang_PrintingPolicy_getProperty",
   [PrintingPolicy, c_uint],
   c_uint),

  ("clang_PrintingPolicy_setProperty",
   [PrintingPolicy, c_uint, c_uint]),
...
class Config:
    ...
    def set_library_path(path):
        ...
        if Config.loaded:
            raise RuntimeError("library path must be set before before using "
    ...
    def set_library_file(filename):
        ...
        if Config.loaded:
            raise RuntimeError("library file must be set before before using "
                               "any other functionalities in libclang.")
    ...
    def set_compatibility_check(check_status):
        ...
        """
        if Config.loaded:
            raise RuntimeError("compatibility_check must be set before before "
